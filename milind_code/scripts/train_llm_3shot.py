...
Score 3: {demo_score[2]}
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "assistant", "content": "Use the following examples to score the text"+demo_prompt},
...
def main():
    if len(sys.argv) != 3:
        print("Sample CLI execution: python train_llm_3shot.py gold_path.json pred_path.json")
        sys.exit(1)
    ip_path = sys.argv[1]
    op_path = sys.argv[2]
    # Read the JSON file
    ...
    random_indices = random.sample(range(len(gold_labels)), 3)
