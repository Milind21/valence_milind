# TODO:vector store and extract relevant
import json
import pickle
import openai
import re
...
Score 2: {demo_score[1]}
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "assistant", "content": "Use the following examples to guide you on how to score the text"+demo_prompt},
...
def main():
    if len(sys.argv) != 3:
        print("Sample CLI execution: python train_llm_2shot.py gold_path.json pred_path.json")
        sys.exit(1)
    ip_path = sys.argv[1]
    op_path = sys.argv[2]
    # Read the JSON file
    ...
    random_indices = random.sample(range(len(gold_labels)), 2)
    
    #select 500 random examples to be sent to gpt
    ...
    demo_text = [texts[i] for i in random_indices]
    demo_score = [gold_labels[i] for i in random_indices]
